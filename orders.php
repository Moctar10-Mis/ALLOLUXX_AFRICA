        ...
        <table>
            ...
            <tbody>
                <?php while($row = $result->fetch_assoc()): ?>
                    <?php
                    $names = !empty($row['names']) ? explode(',', $row['names']) : [];
                    ...
                    <tr>
                        <td>
                            <?php if(isset($images[$i]) && !empty($images[$i])): ?>
                                <img src="<?php echo trim($images[$i]); ?>" alt="<?php echo htmlspecialchars($names[$i]); ?>">
                            <?php else: ?>
                                No image
                            <?php endif; ?>
                        </td>
                        ...
                        <?php if($i==0): ?>
                            <td rowspan="<?php echo count($names); ?>"><?php echo htmlspecialchars($row['payment_method']); ?></td>
                            <td rowspan="<?php echo count($names); ?>"><?php echo htmlspecialchars($row['status']); ?></td>
                            <td rowspan="<?php echo count($names); ?>"><?php echo date("d M Y H:i", strtotime($row['created_at'])); ?></td>
                        <?php endif; ?>
                    </tr>
                    <?php endfor; ?>
                <?php endwhile; ?>
